...
export function AdminDashboard() {
  ...
  const overview = useQuery(
    isAdmin
      ? (api as any).admin.getPlatformOverview
      : (api as any).notifications.getRecentPublic,
  );
  const creators = useQuery(
    isAdmin
      ? (api as any).admin.listAllCreators
      : (api as any).notifications.getRecentPublic,
  );
  const recentDonations = useQuery(
    isAdmin
      ? (api as any).admin.listAllDonations
      : (api as any).notifications.getRecentPublic,
  ...
  async function handleSeedData() {
    if (
      !confirm("This will seed several demo campaigns and donations. Continue?")
    )
      return;
    setSeeding(true);
    try {
      ...
      <div className="border-b border-border/40 bg-card/50 backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded-full bg-fuel/10 text-fuel text-xs font-semibold">
              LIVE
            </span>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              <Activity
                className={`size-3.5 mr-1.5 ${seeding ? "animate-spin" : ""}`}
              />
        ...
        <div className="max-w-7xl mx-auto px-4 flex gap-1 pb-2">
          {(
            ...
            >
              {t}
              {t === "creators" && Array.isArray(creators) && (
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                {
                  label: "Total Raised",
                  value: fmt$(ov.totalAmountCents ?? 0),
                  icon: <Wallet className="size-4" />,
                  accent: true,
                },
                {
                  label: "Total Gallons",
                  value: (ov.totalGallons ?? 0).toLocaleString(),
                  icon: <Fuel className="size-4" />,
                },
                {
                  label: "Active Creators",
                  value: `${ov.activeCreators ?? 0} / ${ov.totalCreators ?? 0}`,
                ...
                <div
                  key={s.label}
                  className={`p-4 rounded-xl border ${s.accent ? "border-fuel/30 bg-fuel/5" : "border-border/40 bg-card/50"}`}
                >
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="p-4 rounded-xl border border-border/40 bg-card/50 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">{ov.last24hDonations}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div className="p-4 rounded-xl border border-border/40 bg-card/50 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Donations</span>
                  <span className="font-semibold">{ov.last7dDonations}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Revenue</span>
                  <span className="font-semibold">
                    {fmt$(ov.last7dAmountCents)}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">MRR</span>
                  <span className="font-semibold">
                    {fmt$(ov.monthlyRecurringCents)}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {recentDonations.slice(0, 5).map((d: any) => (
                    <div
                      key={d._id}
                      className="flex items-center gap-3 p-3 rounded-lg border border-border/30 bg-card/30 text-sm"
                    >
                      ...
                      <Link
                        to={`/${d.creatorSlug}`}
                        className="text-foreground hover:text-fuel transition-colors font-medium truncate"
                      >
                        {d.creatorName}
                      </Link>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-muted-foreground">
                {filteredCreators.length} profiles
              </span>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/30 border-b border-border/40">
                    <tr>
                      ...
                      <th
                        className="px-3 py-2.5 text-xs font-semibold text-muted-foreground text-right cursor-pointer hover:text-foreground select-none"
                        onClick={() => toggleSort("totalGallons")}
                      >
                        <span className="inline-flex items-center gap-1">
                          Gallons
                          <SortIcon k="totalGallons" />
                        </span>
                      </th>
                      <th
                        className="px-3 py-2.5 text-xs font-semibold text-muted-foreground text-right cursor-pointer hover:text-foreground select-none"
                        onClick={() => toggleSort("totalAmountCents")}
                      >
                        <span className="inline-flex items-center gap-1">
                          Raised
                          <SortIcon k="totalAmountCents" />
                        </span>
                      </th>
                      <th
                        className="px-3 py-2.5 text-xs font-semibold text-muted-foreground cursor-pointer hover:text-foreground select-none"
                        onClick={() => toggleSort("lastDonationAt")}
                      >
                        <span className="inline-flex items-center gap-1">
                          Last Donation
                          <SortIcon k="lastDonationAt" />
                        </span>
                      </th>
                      <th
                        className="px-3 py-2.5 text-xs font-semibold text-muted-foreground cursor-pointer hover:text-foreground select-none"
                  ...
                  <tbody>
                    {filteredCreators.map((c: any) => {
                      const badge =
                        VERIFICATION_BADGE[c.verificationStatus] ??
                        VERIFICATION_BADGE.unverified;
                      const goalPct = c.goal
                        ? Math.min(
                            100,
                            Math.round((c.totalGallons / c.goal) * 100),
                          )
                        : null;
                      return (
                        <tr
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <Link
                                  to={`/${c.slug}`}
                                  target="_blank"
                                  className="text-xs text-fuel/70 hover:text-fuel inline-flex items-center gap-0.5"
                                >
                                  /{c.slug}{" "}
                          ...
                          <td className="px-3 py-3">
                            <div className="flex flex-col gap-1">
                              <span
                                className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs border w-fit ${badge.color}`}
                              >
                                {badge.label}
                              </span>
                              {c.isActive ? (
                          ...
                          <td className="px-3 py-3">
                            ...
                            {c.lastDonationGallons && (
                              <div className="text-xs text-fuel">
                                +{c.lastDonationGallons} gal
                              </div>
                            )}
                          </td>
                          <td className="px-3 py-3">
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                onClick={async () => {
                                  ...
                                  toast.success(
                                    c.isActive ? "Deactivated" : "Activated",
                                  );
                                }}
                              ...
                              <button
                                onClick={async () => {
                                  ...
                                  toast.success(
                                    c.isFeatured ? "Unfeatured" : "Featured!",
                                  );
                                }}
        ...
        {tab === "donations" && Array.isArray(recentDonations) && (
          <div className="space-y-3">
            <div className="text-xs text-muted-foreground">
              {recentDonations.length} most recent completed donations
            </div>
            <div className="rounded-xl border border-border/40 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-muted/30 border-b border-border/40">
                  ...
                  {recentDonations.map((d: any) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <Link
                          to={`/${d.creatorSlug}`}
                          className="font-medium hover:text-fuel transition-colors"
                        >
                          {d.creatorName}
                        </Link>
                      </td>
                      <td className="px-3 py-3 text-right font-bold text-fuel">
                        {d.gallons}
                      </td>
                      <td className="px-3 py-3 text-right font-medium">
                        {fmt$(d.amountCents)}
        ...
        {tab === "notifications" && (
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div className="p-5 rounded-xl border border-border/40 bg-card/50 space-y-4">
              <div>
                <h2 className="font-bold text-base flex items-center gap-2">
                  <Bell className="size-4 text-fuel" /> Send Push Notification
                </h2>
                <p className="text-xs text-muted-foreground mt-1">
                  Delivered live to all users via the bell icon — no email
                  needed.
                </p>
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <Input
                    id="notif-title"
                    placeholder="e.g. Big milestone reached!"
                    value={notifTitle}
                ...
                <div>
                  ...
                  <Input
                    id="notif-link"
                    placeholder="https://www.giveagallon.org/..."
                    value={notifLink}
            ...
            <div className="space-y-3">
              <h2 className="font-bold text-base flex items-center gap-2">
                <Megaphone className="size-4" /> Sent Notifications
              </h2>
              {!notifications || notifications.length === 0 ? (
                <div className="p-8 rounded-xl border border-dashed border-border/40 text-center text-sm text-muted-foreground">
