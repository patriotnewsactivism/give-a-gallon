...
function StatTile({
  ...
  return (
    <Reveal>
      <div
        className={`p-5 rounded-2xl border flex flex-col gap-3 ${accent ? "border-fuel/30 bg-fuel/[0.06]" : "border-border/50 bg-card/50"}`}
      >
        <div
          className={`rounded-xl p-2.5 inline-block ${accent ? "bg-fuel/15" : "bg-muted/40"}`}
        >
          <Icon
            className={`size-5 ${accent ? "text-fuel" : "text-muted-foreground"}`}
          />
        </div>
        <div>
          <div
...
export function ImpactPage() {
  ...
  const successRate =
    campaigns > 0 ? Math.round((successful / campaigns) * 100) : 0;
      ...
      <div className="container py-10 sm:py-16 max-w-5xl">
        ...
        <Reveal className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-lg mx-auto">
            Every number here is a real person who got somewhere they needed to
            be. We publish everything — donations, fees, outcomes.
          </p>
        </Reveal>
        ...
        {!stats ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-12">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div
                key={i}
                className="h-32 rounded-2xl bg-card/30 animate-pulse border border-border/30"
              />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-12">
            <StatTile
              icon={Droplets}
              value={gallons}
              label="Gallons Funded"
              sub={`≈ ${Math.round(gallons * 30)} miles of travel`}
              accent
            />
            <StatTile
              icon={DollarSign}
            ...
            <StatTile
              icon={TrendingUp}
              value={Math.round(platformFeeCollected)}
              label="Platform Fees ($)"
              sub="5% funds operations"
            />
          </div>
        )}
          ...
          <div className="rounded-2xl border border-border/50 bg-card/40 p-6 sm:p-8">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {[
                {
                  pct: "~92%",
                  label: "Goes to creators",
                  desc: "Net after PayPal processing",
                  color: "text-fuel",
                },
                {
                  pct: "5%",
                  label: "Platform fee",
                  desc: "Covers operations & development",
                  color: "text-muted-foreground",
                },
                {
                  pct: "~3%",
                  label: "PayPal processing",
                  desc: "Card processing (via PayPal)",
                  color: "text-muted-foreground",
                },
              ].map(item => (
                <div
          ...
          <Reveal className="mb-12">
            ...
            <div className="space-y-3">
              {byCategory.map(cat => {
                ...
                const pct =
                  maxGallons > 0
                    ? Math.round((cat.gallons / maxGallons) * 100)
                    : 0;
                return (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-muted-foreground shrink-0 ml-2">
                          {cat.gallons} gal · {cat.count} campaign
                          {cat.count !== 1 ? "s" : ""}
                        </span>
                      </div>
          ...
          <div className="rounded-2xl border border-fuel/20 bg-fuel/[0.04] p-6 sm:p-8 text-center">
            ...
            >
              ...
              <span className="text-fuel">
                MANY GALLONS CAN CHANGE A LIFE."
              </span>
            </blockquote>
            <p className="text-muted-foreground max-w-md mx-auto">
              We don't stop at the donation. We track what happened because
              someone gave. That's the Give-A-Gallon promise.
            </p>
          </div>
        </Reveal>
      </div>
