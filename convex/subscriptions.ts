  ...
  handler: async (ctx, args) => {
    ...
    if (!planId) {
      const productRes = await fetch(`${paypalBase()}/v1/catalogs/products`, {
        ...
        body: JSON.stringify({ name: `Give-A-Gallon: ${tier.name}`, type: "SERVICE", category: "CHARITY" }),
      });
      ...
      planId = plan.id;
      console.warn(`Created PayPal plan ${planId} — set ${planEnvKey}=${planId} in Convex env.`);
    }
    ...
    const subRes = await fetch(`${paypalBase()}/v1/billing/subscriptions`, {
      ...
      body: JSON.stringify({
        plan_id: planId,
        subscriber: { email_address: email, name: { given_name: args.donorName ?? "" } },
        application_context: {
    ...
    if (!subRes.ok) throw new Error(`PayPal subscription error: ${await subRes.text()}`);
...
export const cancelSubscription = action({
  ...
  handler: async (ctx) => {
    ...
    if (!sub.stripeSubscriptionId) throw new Error("No subscription ID on record");
    const token = await getPayPalToken();
    const res = await fetch(
      `${paypalBase()}/v1/billing/subscriptions/${sub.stripeSubscriptionId}/cancel`,
      { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ reason: "Cancelled by subscriber" }) },
    );
    if (!res.ok && res.status !== 204) throw new Error(`Failed to cancel: ${await res.text()}`);
    await ctx.runMutation(internal.subscriptions.updateSubStatus, {
      stripeSubscriptionId: sub.stripeSubscriptionId, status: "canceled", canceledAt: Date.now(),
    });
    if (sub.donorEmail) {
      await ctx.runAction(internal.emails.sendSubscriptionCanceled, {
        donorEmail: sub.donorEmail, donorName: sub.donorName ?? "there",
        tierName: sub.tierName, currentPeriodEndMs: sub.currentPeriodEnd ?? Date.now(),
      });
    }
    return { success: true };
  },
...
export const getMySubscription = query({
  ...
  handler: async (ctx) => {
    ...
    return ctx.db.query("subscriptions").withIndex("by_user", q => q.eq("userId", userId))
...
export const getSubscriptionStats = query({
  args: {},
  handler: async (ctx) => {
    const subs = await ctx.db.query("subscriptions").withIndex("by_status", q => q.eq("status","active")).collect();
    return {
      ...
      byTier: subs.reduce<Record<string,number>>((acc,sub) => { acc[sub.tierId]=(acc[sub.tierId]??0)+1; return acc; },{}),
    };
  },
});
...
export const getActiveSubForUser = internalQuery({
  ...
  handler: async (ctx, { userId }) =>
    ctx.db.query("subscriptions").withIndex("by_user", q => q.eq("userId",userId))
      .filter(q => q.eq(q.field("status"),"active")).first(),
});
...
export const getSubByPayPalId = internalQuery({
  ...
  handler: async (ctx, { paypalSubscriptionId }) =>
    ctx.db.query("subscriptions").withIndex("by_stripeSubscription", q => q.eq("stripeSubscriptionId",paypalSubscriptionId)).first(),
});

export const createSubscriptionRecord = internalMutation({
  args: {
    userId: v.id("users"), donorEmail: v.string(), donorName: v.optional(v.string()),
    tierId: v.string(), tierName: v.string(), amountCents: v.number(), gallonsPerMonth: v.number(),
    paypalSubscriptionId: v.string(), currentPeriodEnd: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    const now = Date.now();
    const existing = await ctx.db.query("subscriptions").withIndex("by_user", q => q.eq("userId",args.userId)).first();
    if (existing) await ctx.db.patch(existing._id, { status: "canceled", canceledAt: now, updatedAt: now });
    return ctx.db.insert("subscriptions", {
      userId: args.userId, donorEmail: args.donorEmail, donorName: args.donorName,
      tierId: args.tierId, tierName: args.tierName, amountCents: args.amountCents,
      gallonsPerMonth: args.gallonsPerMonth, stripeSubscriptionId: args.paypalSubscriptionId,
      status: "active", currentPeriodEnd: args.currentPeriodEnd, createdAt: now, updatedAt: now,
    });
  },
});
...
export const updateSubStatus = internalMutation({
  args: {
    ...
    canceledAt: v.optional(v.number()), currentPeriodEnd: v.optional(v.number()),
  },
  handler: async (ctx, { stripeSubscriptionId, status, canceledAt, currentPeriodEnd }) => {
    const sub = await ctx.db.query("subscriptions").withIndex("by_stripeSubscription", q => q.eq("stripeSubscriptionId",stripeSubscriptionId)).first();
    if (!sub) return;
    await ctx.db.patch(sub._id, { status, ...(canceledAt?{canceledAt}:{}), ...(currentPeriodEnd?{currentPeriodEnd}:{}), updatedAt: Date.now() });
  },
});
